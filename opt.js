...
(async function boot(){
  ...
  $("#useProxy").checked=!!s.useProxy; $("#autoSumm").checked=!!s.autoSummarize; $("#maxSumm").value=s.maxSummariesPerFeed||3; $("#hideFooter").checked=!!s.hideFooter;
  // calendar max events
  const calMax = (s.maxCalendarEvents==null) ? 10 : s.maxCalendarEvents;
  $("#maxCalEvents").value = calMax;
  async function doSaveAndMaybeBack(navigateBack){
    const ns={
  ...
  if(saveTop) saveTop.addEventListener('click', ()=> doSaveAndMaybeBack(true));
  ...
  // Check for add-from-toolbar query params
  ...
  document.getElementById('importFile').addEventListener('change', async (e)=>{
    const f = e.target.files[0]; if(!f) return; const txt = await f.text(); try{ const obj=JSON.parse(txt); if(obj.settings) await chrome.storage.sync.set({settings:obj.settings}); if(obj.feeds) await chrome.storage.sync.set({feeds:obj.feeds}); if(obj.tools) await chrome.storage.sync.set({tools:obj.tools}); if(obj.icsUrls) await chrome.storage.sync.set({icsUrls:obj.icsUrls}); showToast('Imported'); renderFeeds(); renderToolsManager(); }catch(err){ showToast('Import failed'); }
  });
  ...
  const testSavedKeyBtn = document.createElement('button'); testSavedKeyBtn.textContent='Test saved key'; testSavedKeyBtn.style.marginLeft='8px'; document.querySelector('#aiKey').parentElement.appendChild(testSavedKeyBtn);

  const creds = await load('ai',{provider:'openai',key:''});
  // load custom provider config too
  ...
  customFields.style.display = aiProvider.value === 'custom' ? 'block' : 'none';

  aiProvider.onchange = ()=>{ customFields.style.display = aiProvider.value === 'custom' ? 'block' : 'none'; };

  // If academiccloud preset selected, prefill custom provider fields for convenience
  function applyAcademiccloudPreset(){
  ...
  aiProvider.addEventListener('change', ()=>{
    if(aiProvider.value === 'academiccloud') applyAcademiccloudPreset();
    else if(aiProvider.value !== 'custom') customFields.style.display = 'none';
  });
  ...
  // Test saved key (reads local first, then sync)
    ...
    async function testApi({ key, cfg, source }){
      ...
      async function doFetch(){
        try{
          ...
          // prepare curl with sanitized header name and redacted value
          ...
          setStatus('Error');
          setResp(msg + '\n\nHints: This may be a CORS error (blocked in browser). Try the Cloudflare Worker proxy to avoid CORS or verify server allows your origin.');
        }
      }
  ...
  // Save AI creds when Save top clicked
  const origDoSave = doSaveAndMaybeBack;
  doSaveAndMaybeBack = async (navigateBack)=>{
    await origDoSave(navigateBack);
    const aiConf = { provider: aiProvider.value||'openai', key: aiKey.value||'', storeLocal: !!storeKeyLocal.checked };
    // store key in chrome.storage.local if requested
    if(storeKeyLocal.checked){ await chrome.storage.local.set({ ai_key: aiKey.value||'' }); aiConf.key = ''; }
    await chrome.storage.sync.set({ ai: aiConf });
    await chrome.storage.sync.set({ ai_custom: { base: customBase.value.trim(), path: customPath.value.trim(), authHeader: customAuthHeader.value.trim(), model: customModelInput.value.trim() } });
  };

  // Undo last add (show toast with Undo)
  async function showUndoToast(){
    const el=document.getElementById('toast'); if(!el) return; el.innerHTML = 'Added <button id="undoBtn">Undo</button>'; el.classList.add('show');
    document.getElementById('undoBtn').onclick = async ()=>{ await chrome.runtime.sendMessage({type:'undo_last'}); el.classList.remove('show'); showToast('Undone'); renderFeeds(); renderToolsManager(); };
