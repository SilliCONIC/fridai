...
async function refreshFeeds(){
  ...
  for(const f of state.feeds){
    ...
    try{
      ...
      if(state.settings.autoSummarize){ (updated[f.id]||[]).forEach(async it=>{ if(!it.localSummary){ setTimeout(()=>{ chrome.runtime.sendMessage({type:'summarize_ai', url: it.url}); }, 800); }});
      }
...
async function loadModels() {
  try {
    ...
    modelSelect.value = 'meta-llama-3.1-8b-instruct'; // default
  } catch (e) {
